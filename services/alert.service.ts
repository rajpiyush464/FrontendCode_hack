...
export const alertService = {
  async getAlerts(vehicleId: string): Promise<Alert[]> {
    // 👇 Change "/alerts/vehicle/" to "/vehiclealert/alerts/vehicle/"
    const { data } = await api.get(`/vehiclealert/alerts/vehicle/${vehicleId}`);
    return data.alerts;
  },

  async resolveAlert(alertId: string): Promise<boolean> {
    // 👇 Change "/alerts/" to "/vehiclealert/alerts/"
    const { data } = await api.post(`/vehiclealert/alerts/${alertId}/resolve`);
    return data.success;
  }
};
export default alertService;